...
interface ImageReader {
  ...
  final class ParcelFileDescriptorImageReader implements ImageReader {
    ...
    @Override
    public ImageHeaderParser.ImageType getImageType() throws IOException {
      // FileInputStream doesn't support mark() and ImageHeaderParserUtils wraps the provided
      // InputStream if it doesn't support mark. Here we create a FileInputStream from the
      // parcelFileDescriptor and add support for mark() via calling the associated
      // dataRewinder.
      try (InputStream is = createRewindableInputStream()) {
    ...
    @Override
    public int getImageOrientation() throws IOException {
      // See comment in getImageType() about createRewindableInputStreamFrom().
      try (InputStream is = createRewindableInputStream()) {
        return ImageHeaderParserUtils.getOrientation(parsers, is, byteArrayPool);
      }
    }

    private InputStream createRewindableInputStream() throws IOException {
      return new FileInputStream(dataRewinder.rewindAndGet().getFileDescriptor()) {
        @Override
        ...
        public synchronized void mark(int readLimit) {
          // Don't need to do anything here, as seeking a ParcelFileDescriptor is trivial.
        }
